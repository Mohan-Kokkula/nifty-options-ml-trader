...
print("\n-- in_entry_window (11:30-15:00: window moved, cutoff original) --")
...
check("14:00 allowed (cutoff reverted to 15:00)",
      in_entry_window(datetime(2026, 8, 7, 14, 0), CFG))
check("15:00 REJECTED (cutoff is exclusive)",
      not in_entry_window(datetime(2026, 8, 7, 15, 0), CFG))
...
check("stop = 4.0 x ATR (original, re-confirmed VAL-optimal)",
      abs(CFG.stop_R * CFG.atr_mult - 4.0) < 1e-9)
check("window starts 11:30", CFG.entry_window_start == "11:30")
check("cutoff 15:00 (reverted)", CFG.entry_cutoff == "15:00")
check("max 3 trades/day (reverted)", CFG.max_trades_per_day == 3)
