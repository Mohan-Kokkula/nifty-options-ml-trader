...
top_pos = audit.sort_values("ic", ascending=False).head(5)["feature"].tolist()
top_neg = audit.sort_values("ic", ascending=True).head(5)["feature"].tolist()
...
print()
g_pos = bucket_report(d["pos_score"], d["fwd"], "TOP-5 POSITIVE-IC composite (should predict UP when HIGH)")
g_neg = bucket_report(d["neg_score"], d["fwd"], "TOP-5 NEGATIVE-IC composite (should predict DOWN when HIGH)")
